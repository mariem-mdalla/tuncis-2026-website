...
const ORGANIZER_EMAIL = process.env.NOTIFY_EMAIL || process.env.EMAIL_USER || "tuncis2026@horizon-tech.tn";
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.06);">
            ...
            <tr>
              <td style="padding: 30px 30px 10px 30px;">
                <h2 style="color: #022c5e; margin: 0 0 8px 0; font-size: 20px; font-weight: 700;">${title}</h2>
                ${subtitle ? `<p style="color: #64748b; margin: 0 0 20px 0; font-size: 14px; line-height: 1.5;">${subtitle}</p>` : ''}
              </td>
            </tr>
            <tr>
              <td style="padding: 10px 30px 30px 30px;">${contentHtml}</td>
            </tr>
            <tr>
              <td style="background-color: #f8fafc; padding: 20px 30px; border-top: 1px solid #e2e8f0;">
                <p style="margin: 0 0 6px 0; font-size: 12px; font-weight: 700; color: #022c5e; text-transform: uppercase; letter-spacing: 0.5px;">Venue</p>
            ...
            <tr>
              <td style="background-color: #022c5e; padding: 18px 30px; text-align: center;">
                <p style="color: rgba(255,255,255,0.6); margin: 0; font-size: 12px;">
                  &copy; 2026 TUNCIS &middot; Organized by TunAISia &amp; AIS &middot;
                  <a href="mailto:tuncis2026@horizon-tech.tn" style="color: #FBD53A; text-decoration: none;">tuncis2026@horizon-tech.tn</a>
                </p>
              </td>
            </tr>
          </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse; width: 100%; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; margin-top: 12px;">
      ${rows.map(([label, value], idx) => `
        <tr style="background-color: ${idx % 2 === 0 ? '#f8fafc' : '#ffffff'}; border-bottom: 1px solid #e2e8f0;">
          <td style="padding: 12px 16px; font-size: 13px; font-weight: 700; color: #022c5e; width: 38%; vertical-align: top;">${label}</td>
          <td style="padding: 12px 16px; font-size: 13px; color: #334155; width: 62%; vertical-align: top;">${value || '—'}</td>
        </tr>
      `).join('')}
    </table>`;
}
...
export default async function handler(req, res) {
  ...
  try {
    const { fullName, email, fileBase64, fileName, fileSize } = req.body;
    ...
    if (!fileBase64) {
      return res.status(400).json({ success: false, message: "Please upload your abstract in PDF format." });
    }

    // Reconstruct the PDF buffer from base64
    const fileBuffer = Buffer.from(fileBase64, 'base64');
    const safeFileName = fileName || 'abstract.pdf';
    const fileSizeKb = fileSize ? (fileSize / 1024).toFixed(1) : (fileBuffer.length / 1024).toFixed(1);
    const submissionDate = new Date().toLocaleString("en-US", { timeZone: "Africa/Tunis" }) + " (Tunis Time)";

    const abstractTableRows = [
      ["Author Name", fullName],
      ["Author Email", email],
      ["Attached PDF File", safeFileName],
    ...
    const organizerHtml = wrapEmailHtml({
      ...
      contentHtml: `
        <p style="font-size: 14px; color: #334155; margin-bottom: 12px;">
          The PDF is attached to this email. Submission details:
        </p>
    ...
    const applicantHtml = wrapEmailHtml({
      ...
      contentHtml: `
        ...
        <p style="font-size: 14px; font-weight: 700; color: #022c5e; margin-bottom: 8px;">Submission Summary:</p>
        ${renderTableRows(abstractTableRows)}
    ...
    console.log(`Abstract emails sent: organizer=${ORGANIZER_EMAIL}, applicant=${email}, file=${safeFileName}`);

    return res.status(201).json({ success: true, message: "Abstract submitted successfully." });
  } catch (err) {
    console.error("Abstract submission error:", err);
    return res.status(500).json({ success: false, message: "Failed to submit abstract. Please try again." });
  }
}
